# aichat — Usage Reference

Detailed reference for everything aichat can do. The README covers the
sharp pitch and the two killer demos; this document covers the full
surface.
...
- [MCP tools](#mcp-tools)
- [Local command agents](#local-command-agents)
...
```bash
aichat task "Create a 3-point plan to cut cloud costs" \
  --starter claude --participants claude gpt
```

The conversation streams to your terminal. Use `Ctrl+C` to stop early or
add `--max-turns 6` to limit exchanges. Save the transcript with
`--output plan.md`.
...
`mcp_servers` declares the tool surface assigned to each agent. The
mental model is:
...
The model is the reasoning engine; MCP servers define what tools that
agent may use.

## MCP tools

Install the MCP SDK to inspect and call live MCP servers:
...
Discovery is opt-in because stdio MCP servers execute local commands
from config.
...
`--enable-tool-calls` implies MCP tool discovery. Use
`--max-tool-calls-per-turn` to cap tool activity.

### Filesystem MCP smoke test

This repo includes a tiny read-only filesystem MCP server for local
testing. It only exposes files under the configured root.
...
Use a command-backed agent when you want aichat to talk to another local
CLI program without copy/paste. The command receives the conversation
prompt on stdin unless one of its args contains `{prompt}`.
...
`cwd` is optional. When relative, it resolves from the YAML config
file's directory, so configs run cleanly from any shell location.
...
This is the non-copy/paste bridge for local tools. Fully live terminal
attachment depends on whether the target app exposes a stable CLI, API,
MCP server, or stdio mode.

### Templates for local command agents

The `init` subcommand scaffolds working configs:

```bash
aichat init codex-claude --fresh
aichat init codex-claude --resume
aichat init ollama-codex --fresh
aichat init ollama-codex --resume
aichat init fusion-mcp
```

For local CLI templates, `--fresh` starts a new local assistant session
each run; `--resume` continues the most recent local session when the
...
Use relay mode when one AI needs to draft a message for another AI
assistant or external interface, but a human should approve before
anything is sent.
...
Every proposed relay and human decision is written into the transcript.
This is the safe path for connecting a project-aware model to another
assistant, CAD tool, MCP server, or human-operated workflow.
...
The container uses `/workspace` as its working directory, so mounted
configs and transcripts behave like local files.
...
This writes `haiku.md.classified.jsonl` and prints a summary of factual
assertions, opinions, predictions, recommendations, hypotheticals,
questions, and meta-commentary.
